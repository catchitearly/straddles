...
def generate_interactive_html(data):
    ...
    <head>
        ...
        <style>
            ...
            .metrics-grid {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 10px; margin-bottom: 20px; }}
            ...
            .config-grid {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(110px, 1fr)); gap: 15px; align-items: end; }}
    ...
    <body>
        ...
        <div class="card">
            <div class="config-grid">
                ...
                <div><label>Start Time</label><input type="time" id="startTime" value="11:15"></div>
                <div><label>End Time</label><input type="time" id="endTime" value="13:30"></div>
        ...
        <script>
            ...
            function runSimulation() {{
                const cfg = {{
                    ...
                    startT: document.getElementById('startTime').value,
                    endT: document.getElementById('endTime').value
                }};
                ...
                for (let date in masterData) {{
                    for (let strike in masterData[date].strikes) {{
                        ...
                        for (let i = 0; i < s.data1m.length; i++) {{
                            ...
                            if (!active) {{
                                if (time >= cfg.startT && time <= cfg.endT && m30 && m60) {{
                                ...
                                // Simple TSL Logic
                                if (pft >= 50) active.tsl = Math.min(active.tsl, active.entryPrice - 40);
                ...
                allTrades.forEach(t => {{
                    ...
                    if (t.pnl > 0) {{ wins++; grossWin += t.pnl; currentStreak = 0; }} 
                    else {{ losses++; grossLoss += Math.abs(t.pnl); currentStreak++; }}
                ...
                const mHTML = `
                    ...
                    <div class="metric-item"><div class="metric-label">Sharpe</div><div class="metric-value">${{sharpe.toFixed(2)}}</div></div>
                    <div class="metric-item"><div class="metric-label">Max DD</div><div class="metric-value loss">${{maxDD.toFixed(2)}}</div></div>
                    <div class="metric-item"><div class="metric-label">Trades (W/L)</div><div class="metric-value">${{allTrades.length}} (${{wins}}/${{losses}})</div></div>
                `;
...
if __name__ == "__main__":
    ...
    print("Done! Open 'simulator.html' to view results.")
