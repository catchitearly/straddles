...
def _time_to_expiry_years(candle_time, expiry_dt=None):
    ...
    expiry_dt defaults to the main script's single EXPIRY_DT (used everywhere else -
    straddle Greeks, single-expiry GEX); multi-expiry GEX passes each expiry's own datetime.
    """
...
def fetch_option_chain(fyers, strike_count=GEX_STRIKE_COUNT, timestamp="", _quiet=False):
    ...
    timestamp="" fetches the nearest expiry; pass a specific expiry's epoch
    (as reported by fetch_available_expiries()) to fetch that expiry instead.
    Returns (chain_df, spot_price, expiry_data) or (None, None, None) on failure.
    chain_df has one row per strike: strike, ltp_ce, ltp_pe, oi_ce, oi_pe.
    expiry_data is the raw list of {date, expiry} Fyers reports as available.
    _quiet suppresses the "no CE/PE rows" warning - used by fetch_available_expiries(),
    which only wants expiry_data and requests strike_count=1 just to keep the call cheap.
    """
    try:
...
def fetch_available_expiries(fyers):
    """One lightweight call to discover Fyers' available expiries (date + epoch
    'timestamp' value to use for fetching that specific expiry's chain).
    Returns a list of dicts: {"date": <raw date string from Fyers>, "epoch": <str/int>}.
    """
...
def compute_gex_snapshot(chain_df, spot, expiry_dt=None, as_of=None):
    ...
    expiry_dt defaults to the script's main EXPIRY_DT if not given.
    """
...
def compute_gamma_flip_multi(gex_records, spot, as_of=None):
    """Same as compute_gamma_flip, but for a combined set of (strike, expiry_dt,
    iv_ce, iv_pe, oi_ce, oi_pe) records spanning MULTIPLE expiries - each record
    uses its OWN expiry_dt for time-to-expiry, so the grid reflects the true
    combined dealer gamma across all included expiries.
...
def update_gex_and_get_history(fyers, fallback_spot):
    """Fetch a fresh option-chain snapshot (nearest expiry), compute GEX +
    flip, append it to the persisted history log, and return the full day's
    history so far (list of dicts: time, spot, net_gex, flip). Never raises -
    logs and falls back to returning whatever history already exists on failure.
    """
...
def update_combined_gex_and_get_history(fyers, fallback_spot, expiry_codes=None):
    """Fetch option chains for MULTIPLE expiries (given as Fyers weekly symbol
    codes, e.g. '26707'), compute each expiry's own GEX (with its own time-to-
    expiry), combine them (sum for net GEX; a proper multi-expiry grid search
    for the flip level, not just averaging each expiry's individual flip), and
    append the combined snapshot to a separate persisted history log.
    Returns the full day's combined history so far.
    """
    ...
    try:
        ...
        if not available:
            logger.warning("Could not discover available expiries from Fyers - skipping combined GEX this run.")
            return history
        ...
        combined_flip, _curve = compute_gamma_flip_multi(combined_records, spot)
        ...
        logger.info(f"✓ Combined GEX snapshot: spot={spot:.1f} net_gex={combined_net_gex/GEX_SCALE:.2f} Cr flip={combined_flip} "
                    f"(expiries: {list(per_expiry_net_gex.keys())})")
    except Exception as e:
...
def build_dashboard_html(straddle_data, atm, rankings, gex_history=None, combined_gex_history=None):
    ...
    # --- Combined GEX chart: same idea, but net GEX + flip are aggregated across
    # multiple expiries (GEX_MULTI_EXPIRY_CODES), which is closer to how GEX is
    # normally computed in practice than looking at a single nearest expiry alone. ---
    ...
    if combined_gex_history:
        ...
        fig_gex_combined.add_trace(go.Scatter(
            x=c_times, y=c_spots, name="NIFTY Spot",
            line=dict(color=BLUE, width=2)
        ), secondary_y=False)
        fig_gex_combined.add_trace(go.Scatter(
            x=c_times, y=c_flips, name="Combined Gamma Flip",
            line=dict(color=ACCENT, width=2, dash="dash")
        ), secondary_y=False)
        fig_gex_combined.add_trace(go.Scatter(
            x=c_times, y=c_nets, name="Combined Net GEX (₹ Cr)",
            line=dict(color=RED, width=2.5), fill="tozeroy", fillcolor="rgba(255,69,96,0.08)"
        ), secondary_y=True)
        fig_gex_combined.add_trace(go.Scatter(
            x=c_times, y=[0] * len(c_times), name="Zero GEX",
            line=dict(color=MUTED, width=1, dash="dot"), showlegend=False
        ), secondary_y=True)

        # Thin per-expiry breakdown lines so you can see which expiry is driving the combined number
        ...
        for idx, code in enumerate(expiry_codes_seen):
            per_expiry_series = [
                (h.get("per_expiry") or {}).get(code, None)
                for h in combined_gex_history
            ]
            per_expiry_series = [v / GEX_SCALE if v is not None else None for v in per_expiry_series]
            fig_gex_combined.add_trace(go.Scatter(
                x=c_times, y=per_expiry_series, name=f"Expiry {code} (₹ Cr)",
    ...
    <div class="tab-content" id="tab-gexcombined">
        <div class="metric-card">
            <div style="color:{MUTED};font-size:12px;margin-bottom:12px;">
                ...
                Same live-snapshot-only caveat as the single-expiry GEX tab applies.
            </div>
...
def main():
    ...
    try:
        ...
        if results:
            ...
            docs_path, backup_path = build_dashboard_html(results, atm, rankings, gex_history, combined_gex_history)
