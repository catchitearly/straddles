...
<body>
    ...
    filename = f"{TARGET_DATE}_{datetime.now().strftime('%H%M%S')}.html"
...
def main():
    if not CLIENT_ID or not TOKEN:
        logger.error("API Credentials missing."); return
    ...
    for offset in OFFSETS:
        ...
        if not ce_df.empty and not pe_df.empty:
            ...
            logger.info(f"Fetched Strike {strike}")

    if results:
        path = build_dashboard_html(results, atm, compute_rankings(results))
