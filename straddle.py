...
<head>
    ...
    <style>
        * {{ box-sizing: border-box; }}
        ...
        th {{ color: {MUTED}; text-transform: uppercase; font-size: 10px; padding: 10px; border-bottom: 1px solid {BORDER}; }}
        ...
        #speedTable th {{ color: {MUTED}; font-size: 10px; padding: 10px 14px; border-bottom: 1px solid {BORDER}; white-space: nowrap; }}
        #speedTable td {{ padding: 10px 14px; font-size: 13px; border-bottom: 1px solid {BORDER}22; white-space: nowrap; }}
        #speedTable tr:hover td {{ background: {BORDER}44; }}
        ...
        .time-indicator {{ display: flex; align-items: center; gap: 8px; }}
        .dot {{ width: 8px; height: 8px; border-radius: 50%; background: {ACCENT}; display: inline-block; }}
        .duration-badge {{ background: {BLUE}22; color: {BLUE}; border-radius: 4px; padding: 2px 8px; font-size: 11px; }}
        .strike-dot {{ width: 10px; height: 10px; border-radius: 50%; display: inline-block; margin-right: 6px; }}
    </style>
</head>
<body>
    <div class="header">
    ...
    <div class="speed-card">
        <div class="speed-header">
            <div>
                <h2 style="margin:0;">⚡ SPEED OF ASCENT / DESCENT</h2>
                <div style="color:{MUTED};font-size:12px;margin-top:4px;">Points moved per minute across selected window</div>
            </div>
            <div class="speed-controls">
                <div class="slider-group">
                    <div class="slider-label">Reference Time</div>
                    <input type="range" id="timeSlider" min="0" max="1" value="0" step="1">
                    <div style="display:flex;justify-content:space-between;">
                        <span class="slider-value" id="timeVal">--:--</span>
                        <span class="time-indicator"><span class="dot"></span> <span id="timeDisplay" style="font-size:12px;color:{MUTED};">Select time</span></span>
                    </div>
                </div>
                <div class="slider-group">
                    <div class="slider-label">Duration (candles)</div>
                    <input type="range" id="durationSlider" min="1" max="12" value="3" step="1">
                    <div style="display:flex;justify-content:space-between;align-items:center;">
                        <span class="slider-value" id="durationVal">3</span>
                        <span class="duration-badge" id="durationMins">15 min</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="speed-table-wrap">
            <table id="speedTable">
                ...
                <tbody id="speedTableBody">
                    <tr><td colspan="10" style="text-align:center;color:{MUTED};padding:30px;">
                        Move the sliders above to calculate speed
                    </td></tr>
                </tbody>
            </table>
        </div>
    ...
    // Init time slider max
    timeSlider.max = allTimes.length - 1;
    timeSlider.value = Math.floor(allTimes.length / 2);

    function getPriceAt(strike, idx) {{
        const series = speedData[strike];
        if (!series || idx < 0 || idx >= series.length) return null;
        return series[idx];
    }}

    // Compute smoothness over a slice of prices (mirrors Python _smoothness)
    function calcSmoothness(strike, fromIdx, toIdx) {{
        const series = speedData[strike];
        if (!series) return null;
        const slice = series.slice(fromIdx, toIdx + 1).map(d => d.price);
        if (slice.length < 3) return null;
        const net   = Math.abs(slice[slice.length - 1] - slice[0]);
        let total   = 0;
        for (let i = 1; i < slice.length; i++) total += Math.abs(slice[i] - slice[i-1]);
        return total > 0 ? Math.round((net / total) * 10000) / 100 : 100.0;
    }}

    function smoothnessColor(sm) {{
        if (sm === null) return '{MUTED}';
        if (sm >= 70) return '{ACCENT}';
        if (sm >= 40) return '{BLUE}';
        return '{RED}';
    }}
    ...
    function updateTable() {{
        ...
        const fromIdx   = Math.max(0, tIdx - duration);
        const toIdx     = tIdx;

        const fromTime  = allTimes[fromIdx] || '--';
        const toTime    = allTimes[toIdx]   || '--';
        const mins      = duration * 5;  // each candle = 5 min

        timeVal.textContent     = toTime;
        timeDisplay.textContent = `Window end: ${{toTime}}`;
        ...
        const rows = strikes.map((strike, idx) => {{
            ...
            const deltaColor = delta > 0 ? '{ACCENT}' : delta < 0 ? '{RED}' : '{MUTED}';
            const spdColor   = Math.abs(speed) > 5 ? (delta > 0 ? '{ACCENT}' : '{RED}') : '{MUTED}';
            const atmMark    = strike === ATM ? ' <span style="color:{ACCENT};font-size:10px;">ATM</span>' : '';

            const sm        = calcSmoothness(strike, fromIdx, toIdx);
            const smDisplay = sm !== null ? sm.toFixed(2) + '%' : '--';
            const smColor   = smoothnessColor(sm);
            const smBar     = sm !== null
                ? `<div style="margin-top:4px;height:3px;border-radius:2px;background:{BORDER};width:80px;">
                       <div style="height:3px;border-radius:2px;background:${{smColor}};width:${{Math.min(sm,100)}}%;"></div>
                   </div>`
                : '';

            return `<tr>
                <td>
                    <span class="strike-dot" style="background:${{color}};"></span>
                    <b>${{strike}}</b>${{atmMark}}
                </td>
                <td style="color:{MUTED};">${{fromPt.time}}</td>
                ...
                <td style="color:${{smColor}};font-weight:bold;">${{smDisplay}}${{smBar}}</td>
                <td>${{badge}}</td>
            </tr>`;
        }}).filter(Boolean);

        if (rows.length === 0) {{
            tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;color:{MUTED};padding:30px;">No data available for selected window</td></tr>';
        }} else {{
    ...
    durationSlider.addEventListener('input', updateTable);
